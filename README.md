...
## 06/02/2025 (6) - Delegated Proof of Stake

```
[DPoS, 위임 지분 증명]
  - Pos의 변형된 합의 알고리즘으로 높은 효율성과 확장성을 제공
  - 대표자(Delegate)를 선출하여 블록 생성과 검증을 맡기고,
    사용자가 이 대표자를 신뢰할 수 있도록 투표와 위임 메커니즘을 활용
...
```
[Miner]
  - 통상적으로 PoW 합의 알고리즘에서 블록 생성 및 보상을 수행하는 노드를 채굴자(miner)라고 하고,
    PoS방식 등 전력 소모를 통해 블록을 생성하지 않는 다른 합의 알고리즘에서는 블록 제안자(proposer)라고 함
...
## 07/02/2025 (1) - TURING COMPLETENESS
...
## 07/02/2025 (3) - ETHEREUM vs BITCOIN
...
[EVM]
  - Solidity 코드를 solc를 통해 컴파일 -> EVM이 이해할 수 있는 바이트코드로 변환 -> 이더리움 네트워크로 배포
